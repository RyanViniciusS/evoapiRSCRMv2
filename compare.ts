    ...
    'messages.upsert': async (
      ...
      try {
        for (const received of messages) {
          console.log('TYPE:', type, 'hasMessage:', !!received?.message, 'status:', received?.status);
          const key: any = received.key;

          // if (key?.remoteJid?.endsWith('@lid') && key?.remoteJidAlt) {
          //   key.remoteJid = key.remoteJidAlt;
          // }
          // console.log("Ryan", key?.remotJid)
          // console.log('RAW MESSAGE RECEIVED:', JSON.stringify({
          //   type,
          //   messageKeys: received?.message ? Object.keys(received.message) : [],
          //   fullMessage: received?.message,
          //   key: received?.key,
          // }, null, 2));
